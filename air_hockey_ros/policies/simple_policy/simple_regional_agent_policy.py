...
class SimpleRegionalAgentPolicy(AgentPolicy):
    def __init__(self, id, p_radius, x_min, x_max, y_min, y_max, step_size):
    ...
    def update(self, world_state) -> Tuple[int, int]:
        ...
        if valid_x and valid_y:
            if agent_x < puck_x - self.p_radius:
        ...
        if x_diff < 0 and x_diff > -self.p_radius and y_diff_abs < self.p_radius:
            if y_diff == 0:
                y = 1
            else:
        ...
        if valid_x:
            ...
            if ((x == 1 and agent_x >= self.x_max - self.step_size) or
                    (x == -1 and agent_x <= self.x_min + self.step_size)):
                return 0, y
        if valid_y:
            y = norm_down_num(y_diff)
            if ((y == 1 and agent_y <= self.y_max - self.step_size) or
                    (y ==-1 and agent_y >= self.y_min + self.step_size)):
                return x, 0
        return x, y
    ...
    def up_puck_action(self, agent_x, agent_y, puck_x, puck_y, puck_vx, puck_vy,
        ...
        rx0 = agent_x - (puck_x - self.p_radius)
        ry0 = agent_y - (puck_y - self.p_radius)

        agent_possible_vx = [(0, 0)]
        if agent_x != self.x_min:
            agent_possible_vx.append((-1, -self.step_size))
        if agent_x != self.x_max:
            agent_possible_vx.append((1, self.step_size))
        agent_possible_vy = [(0, 0)]
        if agent_y != self.y_min:
            agent_possible_vy.append((-1, -self.step_size))
        ...
        for agent_x_cmd, agent_vx in agent_possible_vx:
            for agent_y_cmd, agent_vy in agent_possible_vy:
                ...
                if favor_behind:
                    behind_now = agent_x < (puck_x - self.p_radius)
                    ...
                    puck_back_x_t = (puck_x - self.p_radius) + puck_vx * t_star
    ...
    def down_puck_action(self, agent_x, agent_y, puck_x, puck_y) -> Tuple[int, int]:
        ...
        if abs_y_diff > self.p_radius + self.step_size:
            y = int(y_diff/abs_y_diff)
            if ((y == 1 and agent_y >= self.y_max - self.step_size) or
                    (y == -1 and agent_y <= self.y_min + self.step_size)):
                y = 0
            if agent_x <= self.x_min + self.step_size:
                return 0, y
            return -1, y
        if abs_y_diff == self.p_radius + self.step_size:
            if agent_x <= self.x_min + self.step_size:
        ...
        x_diff = puck_x - agent_x
        if abs_y_diff > self.step_size:
            if x_diff >= 0:
                return -1, int(y_diff/x_diff)
            if x_diff < -self.p_radius or agent_x <= self.x_min + self.step_size:
